...
Python 用**縮排**（通常是 4 個空格）來分辨：「哪些程式碼是屬於這個 `if` 的」。如果你忘了縮排，Python 會直接報錯。
...
兩種寫法都是正確的。第一種更精簡，第二種更直觀（對應流程圖的每個菱形）。
...
2000 年是閏年（能被 400 整除），但如果你只寫了「能被 100 整除就是平年」，2000 就會被判成平年。
