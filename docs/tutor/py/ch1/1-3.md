...
ax² + bx + c = 0 有幾個實數根？高一數學告訴你：看判別式 D = b² − 4ac。D > 0 有兩個相異實根、D = 0 有一個重根、D < 0 無實數根。你已經會用紙筆算了，現在讓電腦幫你算！把數學公式翻譯成 Python 運算式，再用 `if-elif-else` 判斷，一氣呵成。

💡 **提示**：Python 還沒教 `**` 運算子（那是後面的事），但 b² 可以用 `b * b` 來算。先算 D，再用 `if-elif-else` 判斷。
