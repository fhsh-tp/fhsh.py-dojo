...
**題目說明**：輸入一個正整數 N（N ≥ 2），判斷 N 是否為質數（只能被 1 和自身整除）。
...
> [!NOTE] 老師的提示
> 從 2 到 n-1 嘗試整除，遇到能整除的就立刻 `break`（表示不是質數）。用一個 `found` 旗標記錄是否找到因數。或者，一旦找到因數就直接輸出 `No` 並結束迴圈。
...
**題目說明**：找出 1 到 N 之間所有的完美數（真因數之和等於自身），並依序印出。
...
**題目說明**：輸入正整數 N（N ≥ 2），找出 N 的最小質因數並輸出。若 N 本身是質數，輸出 N。
...
> [!NOTE] 老師的提示
> 從 2 開始嘗試整除 N，找到第一個能整除的數就 `break` 並輸出。如果跑完 2 到 N-1 都沒找到，表示 N 本身是質數。這道題最適合練習「找到就立刻 break」的思維。
