...
starter_code: |
  ...
  # TODO: 算出被最多台吊扇吹到的那一格，被幾台吊扇吹到
  # 提示：逐台把矩形裡的每一格都加一會太慢，
  #       想想看能不能只在矩形的邊界做記號，最後再一次累加還原
...
- $F$ 可以是 1：只有一台吊扇，那塊矩形裡的每一格都被 1 台吹到，答案就是 `1`。
- 兩台吊扇的風域可以完全重疊（左上角、高、寬都一樣），這時整塊矩形都被 2 台吹到。
- $h_i$ 與 $w_i$ 都可以是 1：風域只蓋住一格。
- 地板上可能有一整片區域完全吹不到，那些格子被吹到的台數是 0；答案只看最多的那一格，不受影響。
- 吊扇數與地板格數都不小：逐台把矩形裡的每一格加一，總步數會是所有矩形面積的總和，大測資會慢到跑不完。
