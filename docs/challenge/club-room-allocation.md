...
starter_code: |
  ...
  def main():
      data = sys.stdin.read().split()
      n = int(data[0])
      starts = list(map(int, data[1:1 + n]))
      durations = list(map(int, data[1 + n:1 + 2 * n]))

      # TODO: 依開始分鐘由早到晚處理每筆申請，開始分鐘相同者依申請編號由小到大
      # TODO: 每筆申請發給當下空著的教室中編號最小的一間，全滿就新開一間
      # TODO: 第一行印出最少要開的教室數
      # TODO: 第二行依「輸入順序」印出每筆申請分到的教室編號，以單一空白分隔
...
- $N$ 可以是 1：只有一筆申請，必定只要開 1 間教室，輸出的第二行就是單獨一個 `1`。
- 兩筆申請的開始分鐘可能完全相同。依規則 2，此時申請編號小的那一筆先挑，會拿到編號較小的教室。
- 一筆申請剛把教室交還、另一筆就從同一分鐘開始，這兩筆**不算重疊**，可以共用同一間教室（範例的第 1 筆與第 3 筆正是這個情形）。
- 多筆申請的開始分鐘與借用時長可能都一模一樣；它們仍依申請編號由小到大，各自分到不同的教室。
- 教室編號永遠是從 1 開始的連續整數：因為每次都取編號最小的空教室，已開教室的編號集合恆為 1 到目前間數的連續一段。
