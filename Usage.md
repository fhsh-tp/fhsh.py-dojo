...
題目頁的內文一律用 **LaTeX** 寫數學陳述，不要用 `≤`、`×`、`2^(D−1)` 這類記號直接打在文字裡。站台的 MathJax 是開著的，`$…$` 會被排版成公式。

`scripts/latex-notation.test.ts` 會擋下違規的寫法，`pnpm test` 就會失敗，訊息會指名檔名、行號與踩到的記號。

判斷順序只有一步：**先問「這串字元會不會原封不動出現在題目的輸入或輸出裡」**——會，就用反引號；不會，再問「它是不是一個數學陳述」——是，就包成 LaTeX；兩者都不是，維持原樣。

| 語境 | 這樣寫 | 別這樣寫 | 取自 |
| :--- | :--- | :--- | :--- |
| 不等式、約束 | `$1 \le T \le 3$` | `1 ≤ T ≤ 3` | `prize-order-code` |
| 約束不要包在反引號裡 | `$1 \le n \le 1000$` | `` `1 <= n <= 1000` `` | `ap-layout-plan` |
| 裸的大於小於也要包 | `$x > 0$ 且 $y < 0$` | `x > 0 且 y < 0` | `quadrant-classifier` |
| 中文當運算元時只包數學片段 | `付款 $<$ 價格` | `付款 < 價格` | `vending-change` |
| 除法 | `$362880 \div 12 = 30240$` | `362880 ÷ 12 = 30240` | `fair-token-exchange` |
| 算式 | `$N \times (N-1) \times \cdots \times 2 \times 1$` | `N×(N−1)×…×2×1` | `rank-code-backfill` |
| 賦值 | `以 $N = 10$ 為例` | `以 N=10 為例` | `prize-order-code` |
| 指數 | `$2^{D-1}$`、`身高(m)$^2$` | `2^(D−1)`、`身高(m)²` | `bmi-classifier` |
| 大數 | `$10^9$` | `1000000000` | `prize-order-code` |
| 負數 | `$-2$` | `−2`（U+2212） | `ap-layout-plan` |
| 散文中的變數 | `輸出 $n$ 行` | ``輸出 `n` 行`` | `ap-layout-plan` |
| 輸入行格式 | `` `D I` `` | `$D\ I$` | `pinball-track-predict` |
| 要印出的字串 | `` `Two Real Roots` `` | `$Two Real Roots$` | `quadratic-discriminant` |
| 圖表裡的字面符號 | `` `·` `` | `$\cdot$` | `print-farm-schedule` |
| 貨幣 | `\$150` | `$150` | `movie-ticket` |

**維持原樣、不要轉的東西：**

- **純散文裡的數字**。「模式 1」就是「模式 1」，不是「模式 $1$」；「去掉尾端的 0 得到 9」也不動。只有式子的一部分才進 LaTeX。
- **波浪號區間**。`1~9`、`3 ~ 90` 是中文排版的寫法，不是數學記號。
- **圖片的 `alt` 文字**。`alt` 不經 Markdown 渲染，寫 LaTeX 只會讓讀者看到原始碼。
- **範例輸入／輸出區塊**（三個反引號包住的部分）。那是字面資料。

**中文不要塞進 `$…$`。** MathJax 的 CJK 字重與內文不同，`\text{體重}` 會看起來不對。公式含中文詞時只把數學片段包起來：

```markdown
BMI = 體重(kg) / 身高(m)$^2$
```

**常用拼法**：`\le`、`\ge`、`\ne`、`\times`、`\div`、`\cdot`、`\cdots`、`\lfloor`、`\rfloor`。負號用 ASCII 的 `-`，不要用 U+2212 的 `−`。大於小於直接寫 `>` 與 `<`，包進 `$…$` 就好。

**逃生門**：真的需要在散文中引用含 `<=`、`>=` 或裸的 `<`、`>` 的程式片段時，在那一行前面加上標記註解，該行就會跳過檢查：

```markdown
<!-- latex-lint-ignore-next-line -->
Python 裡寫成 `if score <= 60:` 就會進到這個分支。
```

標記只作用於緊接的下一個非空行，而且它本身在 diff 裡看得見、審查得到。
