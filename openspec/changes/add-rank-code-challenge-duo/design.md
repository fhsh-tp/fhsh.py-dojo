...
- 568：天真解 O(T×N)（重案 20 筆查詢即破 10M）vs 正解 O(maxN+T)（1.31M）。
- 10212：天真解 O(N)（band N ≥ 10⁸ → ≥5.5×10⁸ ops）vs 正解 O(T×M)（1.67M）。
- 正解 op 餘裕下限定為 5×（實測 7.6×／6.0×）；天真解在斷崖筆需 ≥2× 上限（實測遠超）。
...
兩題各 20 筆：第 1 筆 literal＝題面範例；暖身 band（小值域，天真解可得部分分）；壓力 band（斷崖筆：568 為 T 400..500×N 150000..200000；10212 為 N 10⁸..10⁹×M 80000..100000）；邊界 literal（568：N=1、N=200000 恰界；10212：M=0、P(25,1)=5、P(26,2)=5、M=100000 恰界＝N 同值全乘）。斷崖筆數 ≥6，使繞道總牆鐘遠超預算（D6）。
...
- 10212 區間積：c2、c5 必須分開追蹤；c2>c5 → r×2^(c2−c5) mod 10；c5>c2 → 答案必為 5；相等 → r。M=0 → 1。
...
- 若學生的「正解」實作常數項異常肥大（>10 ops/迭代），在壓力 band 有機會逼近 op 上限——以正解餘裕 ≥5×（D2）與 audit 期第三方實作覆測緩解。
