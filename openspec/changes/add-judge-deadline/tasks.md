...
- [ ] 4.4 量測**所有**已上線題目 spec 中記載為收編的路線在現行 20 筆計畫下的逐筆牆鐘。每條路線一律取**最便宜的合理寫法**（RCA I-16）。驗收目標：逐筆數字寫入 design，且不修改任何題目的測資。對應 spec 需求：Bypass acceptance after hunt downgrade。
  ...
  - [ ] `expression-eval-challenges/spec.md:75` 登記的 4 條收編路線——**零量測**。此項一度被連同整條 4.4 誤標為完成；未量完之前，design〈釘定〉表中的收編上界 3,115 ms 與 1.61 倍餘裕都只是下限。
