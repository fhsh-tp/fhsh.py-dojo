...
- [x] 1.2 [P] 以離線腳本產生三份巢狀對消字串素材（L=20000 兩用途、L=60000 獵殺用；w=ab 交錯前綴＋反轉串接，全滅殘量 0），並以 stack 正解驗證殘量為 0；驗證：素材長度分別為 20000/60000、殘量檢查輸出 0。（⚠ 已由 design.md Decisions 2 降級條款取代：60KB 獵殺筆實測無效已移除，終態為 3 筆兩兩相異的 20KB 巢狀 literal、題面上限 1~40000；本條保留原文作歷史紀錄）
...
- [x] 3.1 撰寫素養題面（消除遊戲測試員情境、二消規則敘述、動手推演＝範例一逐步、輸入說明含 T<=3/N<=5/版面長度 1..60000、輸出說明含 0 為合法答案、「部分測資的版面非常長」效率暗示、範例一與 plan 第一筆逐字元一致）；驗證：全文與 tags、description 以 grep 確認不含 stack、堆疊、資料結構、deque、佇列字樣。（⚠ 已由 design.md Decisions 2 降級條款取代：60KB 獵殺筆實測無效已移除，終態為 3 筆兩兩相異的 20KB 巢狀 literal、題面上限 1~40000；本條保留原文作歷史紀錄）
...
- [x] 4.3 dev 真機驗證（pnpm dev＋瀏覽器）：正解 20/20 AC、天真解 A 壓力筆 TLE、replace 繞法於 60KB 獵殺筆計時——逾 5 秒保留獵殺筆，未逾則依降級條款改為第三筆 20KB 巢狀 literal 並將實測數字補記 design；驗證：三種提交的 verdict 截圖或逐筆結果記錄於 change 目錄筆記。（實測結果：60KB 筆牆鐘 6984ms 但 verdict AC——軟旗標對同步碼結構性失效，已依降級條款改為第三筆 20KB 巢狀 literal；數據見 dev-verification-notes.md）
