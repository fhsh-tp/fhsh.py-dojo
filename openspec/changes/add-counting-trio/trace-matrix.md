...
> **警語（(c) 段的本質限制）**：位置欄的機械檢查**只驗標記存在與否，不驗標記所在那段文字的內容**。`(trace E4)` 出現在 spec.md 就算通過，即使它旁邊那句話把 op 數寫成任何值。「標記旁邊的數字對不對」由 (d) 段（文件數字綁鍵）覆蓋；**非數字的語意誤述**（把「最高換到的級數」寫成「成功兌換的次數」）**沒有任何機械檢查抓得到**，那是正則工具的本質限制。看到位置欄一片 ✓ 不等於文件內容正確。

### (d) 文件數字例外表

`scripts/trace-lint.py` 的 (d) 段會讀 spec.md 與 design.md：**帶標記的每一句，句中每個數字都必須等於該句所標 fact 的某個位址解析值**。真的無法綁鍵的數字（I/O 契約的邊界值、序數、公式分支常數）必須逐筆登記在下表，四欄缺一不可、理由不得留白；**登記了卻沒被用到的例外會以 `doc-number-exempt-stale` 報錯**，例外表因此不會爛在那裡。

登記一筆例外＝公開宣告「這個數字不受機械保護」。加一列前先問：這個數字是不是其實有鍵可指？有就去指鍵，不要來登記。

| 文件 | fact | 數字 | 理由 |
|---|---|---|---|
| spec.md | D7 | 1 | I/O 契約的 n 下界。`measure/routes015.json` 只登記 `n_max`，下界無對應鍵，且它是契約值不是量測值 |
| spec.md | E3 | 1 | 「Entry 1」是序數（第幾筆），不是被主張的量測值；該筆的**內容** 8 已由 `routes015.json#entries[0]` 綁住 |
| spec.md | E9 | 3 | `k < 3` 是封閉式的分支常數，出處為 E1；E1 的交叉驗證只印 stdout，全 change 無可解析位址（見〈尚未有可解析位址的事實〉） |
| spec.md | E9 | 20 | 「x/20」的分母是滿分基數，出處為 E2 的 `entry_count`。E9 列刻意只掛四條路線得分的四個位址，好讓 (b) 段以「／」逐一對位——多掛一個位址會讓段數與位址數不吻合、退回聯集比對，反而變弱 |
| spec.md | F2 | 0 | 契約要求涵蓋 `k = 0`。這是計畫的**涵蓋條件**不是量測結果；`report016.json` 只登記 k>0 的筆數（`entries_k_pos`），沒有登記 0 本身 |
| spec.md | F2 | 1 | 同上，契約要求涵蓋 `n = 1` |
| spec.md | F5 | 0 | 該句的 0 全部出自「不得要求 0/20」與「涵蓋 k = 0」兩個契約條件，不是量測值 |
| spec.md | G2 | 100000000 | 「至少五筆 ≥ 10⁸」的門檻值。`report017.json` 只登記符合的**筆數** `big_entries`＝5，沒有登記門檻本身 |
| design.md | S9 | 20 | S9 只主張三題 frontmatter 的鍵集合一致（15 鍵），「20-entry」是 E2／F2／G2 的事實，在該句中沒有對應位址 |
| design.md | F5 | 0 | 同 spec.md 的 F5：`covering k=0` 是契約涵蓋條件 |
...
| D4 | apcs015 標 `difficulty: easy`，題面附小規模答案表，**不說破** 2×3 區塊。答案表原為 k=1..5，2026-08-15 維護者裁決縮為 **k=1..3**，目的是**少給提示、逼學生自己推**，而非防堵插值——後者已由 E12 證明題面改動無效 | 2026-08-15 訪談 Q4 與後續裁決 | 難度、範例表 | ✓ | ✓ |
...
| C5 | 牆鐘一律**重複 7 次取最小值**。單發量測會抓到排程尖峰：上一輪曾量到「比同路線更大的 n 還慢」的值，物理上不可能（該次觀察的絕對值與倍率都只存在於過程紀錄，見證據欄；主張欄刻意不寫倍率，因為它無出處也無法受檢查） | `curation/plan015.py` 的 `WALL_REPS=7`（原始碼常數）；登記值為 `measure/routes015.json#wall_clock_reps` 與 `measure/routes015.json#wall_clock_statistic`。「上一輪曾量到 803 ms」為過程觀察，**無持久化輸出** | — | ✓ | ✓ |
...
| E12 | **多項式插值是 ACCEPTED ALTERNATIVE，且題面改動關不掉它。** 第 k 列答案在整個定義域上是 k 的四次多項式，5 個資料點就唯一決定它。兩種取點方式皆 20/20：用題面公開值取點、以及**完全不看題面、自己對小 k 暴力枚舉造點**。後者證明縮小或刪除題面給的數字對這條路線毫無影響——學生自己造點只需幾百次配對比較。題面的範例輸出本身就印出 8 個值，超過所需的 5 個，而該範例受 E3 契約約束必須與第一筆 literal 逐位元組相同，無法移除 | `measure/interp-route.json#variants[0].score`、`measure/interp-route.json#variants[1].score`（皆滿分，滿分基數見 `measure/interp-route.json#variants[0].of`）；`measure/interp-route.json#variants[1].max_ops` 與 `measure/interp-route.json#variants[1].op_margin`；`measure/interp-route.json#points_needed_for_quartic`、`measure/interp-route.json#page_example_values_published`、`measure/interp-route.json#page_change_can_close_route`（false）。路線檔為 `curation/routes/r015_interp_selfgen.py`，量測由 `measure/interp_probe.py` 產生 | — | ✓ | ✓ |
...
| F6 | 「忘記取餘數」得 6/20，正確的 6 筆全落在 `n−k ≤ 29`（`2²⁹ < 1000000007`）。**不正確的筆分成兩種死法，不是一種**：9 筆因整數轉字串位數上限拋 `ValueError`（吵鬧的死法），另 5 筆印得出一個較短、可列印但值是錯的整數（安靜的 WRONG_ANSWER）。6 ＋ 9 ＋ 5 才等於 20 | 得分 `measure/report016.json#scores.W4_nomod`、滿分 `measure/assemble_report.json#challenges[1].entry_count`；三分類計數為 `measure/routes016.json#routes.W4_nomod.nomod_correct_count`、`measure/routes016.json#routes.W4_nomod.nomod_exception_count`、`measure/routes016.json#routes.W4_nomod.nomod_wrong_no_exception_count`，皆由 measure016.py 的 `nomod_outcomes()` 從 20 筆實跑結果導出（該函式另斷言三類互斥且窮盡、兩類皆非空，並與 plan016.py 的預測 `nomod_unprintable_entries` 交叉比對；負向控制見 `python3 measure/measure016.py --self-test`）。三類的**逐筆身分**見同一個 `routes.W4_nomod` 底下的 nomod_correct_entries／nomod_exception_entries／nomod_wrong_no_exception_entries 三個鍵——刻意不列為位址，否則 entry 序號會變成計數的候選值，(b) 段就綁不緊 | — | ✓ | — |
...
| G8 | 該路線在 n ≥ 10⁸ 於瀏覽器**必定不可行**，支柱是**記憶體**不是時間：n=10⁸ 的大整數本身即 314,159,123 bytes（約 300 MiB），n=10⁹ 為 3,556,832,228 bytes | n=10⁸ 為 `measure/routes017.json#memory_pillar.rows[2].bytes_of_bigint`（＝`measure/routes017.json#unclean_probe.extrapolated.100000000.bytes_of_bigint`，MiB 值 `measure/routes017.json#memory_pillar.rows[2].mib_of_bigint`）、n=10⁹ 為 `measure/routes017.json#memory_pillar.rows[3].bytes_of_bigint`，兩列的 n 值分別為 `measure/routes017.json#memory_pillar.rows[2].n` 與 `measure/routes017.json#memory_pillar.rows[3].n`，由 measure/routes017_measure.py 的 `bigint_footprint()` 產生、`measure/routes017.json#memory_pillar.deterministic` 為 true。時間外推**不作結論支柱**：同一份資料記錄的對數斜率為 `measure/routes017.json#unclean_probe.loglog_slope_central_from_minima`＝1.746、`measure/routes017.json#unclean_probe.loglog_slope_envelope`＝[1.672, 1.814]，區間本身即涵蓋 8.49% 的離散度（**注意：這幾個數字是牆鐘導出值，每次重跑都會變**——同一台機器連續三次執行，兩個端點逐次不同，只有中心值到小數第一位穩定。把端點逐位釘進矩陣，等於每量一次就失準一次；宜改記中心值到小數第一位，或改記一個帶容差的區間帶。）；先前記載的「1.769 vs 重跑 1.741，差 21%」在 repo 中查無出處，已作廢 | — | ✓ | — |
...
| S7 | apcs015 題面的效能提醒寫「超出單筆測資的**執行量上限**」。**先前寫成「時間限制」是錯的**：E8 證明該路線死於 op 上限。措辭刻意不指名是哪一種限制的細節，避免洩漏系統以什麼為成本軸 | 措辭本體在 docs/challenge/ap-layout-plan.md 的提醒段。2026-08-15 為這條規範補上機械防線——`verify/check_frontmatter_pair.py` 的 **G 檢查**（只掃題面內文，不掃 frontmatter）：時間類措辭掃描結果為 `verify/frontmatter-pair.json#apcs015.performance_hint.timing_terms`（空陣列＝零命中），反向要求「談成本天花板的提醒段必須逐字寫『執行量上限』」的結果為 `verify/frontmatter-pair.json#apcs015.performance_hint.uses_op_limit_phrase`（true），受此約束的段數為 `verify/frontmatter-pair.json#apcs015.performance_hint.cost_hint_paragraphs`。負向控制實跑：把該句改回「時間限制」rc=1、改成含糊的「超出單筆測資的上限」也 rc=1；掃描器另有 `timing_self_test()` 的 8 個注入 probe 與 3 個乾淨對照（含「同一時間」「每秒」等合法用法，證明不誤殺）。判斷依據為 E8 的瀏覽器實測與 op 量測（位址見 E8 列） | 效能提醒 | ✓ | ✓ |
...
| C6／S3／W8 | 三列的證據**全為散文、零可解析位址**：C6 指向 proposal.md 的 Non-Goals 段、S3 的 A′ 檢查只回布林、W8 是 `package.json` 兩條 script 的差異加一次手動對照實驗。與 S7 同型，是本表先前的漏列（規約第 5 條要求列入，實際沒列） | C6 不補（專案層級既有殘留，本 change 的 Non-Goal，只能檢查有無被引用）；S3 把 A′ 的布林落成 `verify/frontmatter-pair.json` 的一個鍵；W8 由一支比對兩條 script 的小腳本落檔 |
| D 段全部 | 本質為維護者決策，無外部真值 | 不補。只能檢查「有沒有被引用」，不能檢查「對不對」 |

2026-08-15 已關閉的缺口（原本列在本表，現已有位址）：**E7 全列**與 **C3 的第三支工具**（`measure/opprobe_015_nbound.py --json` → `measure/nbound015.json`）；**S1／S4／S5／S6／S9**（`verify/check_frontmatter_pair.py --json-out` → `verify/frontmatter-pair.json`，F 檢查已納入）；**S7**（同一支腳本新增的 G 檢查，結果落在 `verify/frontmatter-pair.json` 的 `performance_hint`——在此之前 S7 的證據欄寫著「散文，無持久化輸出」卻沒被列進本表，是本表自身的漏列，已隨 G 檢查一併關閉）。
