...
| W5 | 分頁與 worker **會復原**：不乾淨死亡後緊接著提交正解，兩題皆立刻回到 20/20 | `measure/browser-verification.jsonl#label=ref015_after_unclean.score`（20，滿分為 `measure/browser-verification.jsonl#label=ref015_after_unclean.rows`）與 `measure/browser-verification.jsonl#label=ref015_after_unclean.max_ms`（368 ms）；`measure/browser-verification.jsonl#label=ref017_after_unclean.score`（20）與 `measure/browser-verification.jsonl#label=ref017_after_unclean.max_ms`（3 ms）。**更正**：本列先前宣稱 `ref017_after_unclean` 的前一筆為 `factorial017_pre_recovery`，該 label 在 jsonl 中**不存在**（2026-08-15 全檔 14 筆逐行掃描確認）；實際前一筆是 `measure/browser-verification.jsonl#label=factorial015_rerun`（0/20 的不乾淨死亡），apcs017 的不乾淨死亡則記在更早的 `measure/browser-verification.jsonl#label=factorial017`。「復原測試緊接在不乾淨死亡之後」這個時序主張目前**只由 jsonl 的行序支撐，沒有時間戳可查** | — | ✓ | ✓ |
