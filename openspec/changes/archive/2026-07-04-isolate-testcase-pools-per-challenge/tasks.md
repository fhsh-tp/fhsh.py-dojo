...
- [x] 5.3 推上 staging branch 觸發 Cloudflare Pages build，部署完成後重新驗證 multiplication-table 與另 2 個隨機抽樣受害題目。驗證：staging 上執行對話框預填 stdin 行數 = 該題 params 個數；提交範例答案得 5/5 AC；Cloudflare Pages build log 顯示 `build:pools` 寫出 ≥ 54 個 `.bin`。（2026-07-04 於 `https://staging.fhsh-py-dojo.pages.dev` 完成驗證：全 18 題〔13 受害 + 5 倖存〕stdin 行數與 5/5 AC 兩項檢查皆通過，超出最低抽樣要求。）
