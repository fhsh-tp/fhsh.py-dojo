...
- 新增兩道 apcs／competition 題目，同一個「遊戲排行榜」宇宙（F16、F17）：
  ...
  - `prize-order-code`「頒獎順位驗證碼」（hard）：T ≤ 3 筆查詢，每筆求 N×(N−1)×…×(N−M+1) 去尾零末位（N ≤ 10⁹、M ≤ 100000）。正解 O(M) 區間模運算（出貨 reference 實測 2.33M ops，餘裕 4.3×，F10）；1..N 全乘天真解 op 必破限（F12）；Python 層大數連乘繞道死於總預算硬殺、str() 路線踩 4300 位上限 ValueError（F3、F11）；math.perm＋Legendre 尾零計數的 C 內建路線經實測存活（20/20 AC）且不可由測資消滅，經 RCA 裁決收編為 accepted 聰明解（F19、F20）。
