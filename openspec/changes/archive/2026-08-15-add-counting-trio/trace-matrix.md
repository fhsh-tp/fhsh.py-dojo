...
| G8 | 該路線在 n ≥ 10⁸ 於瀏覽器**必定不可行**，支柱是**記憶體**不是時間：n=10⁸ 的大整數本身即 314,159,123 bytes（約 300 MiB），n=10⁹ 為 3,556,832,228 bytes | n=10⁸ 為 `measure/routes017.json#memory_pillar.rows[2].bytes_of_bigint`（＝`measure/routes017.json#unclean_probe.extrapolated.100000000.bytes_of_bigint`，MiB 值 `measure/routes017.json#memory_pillar.rows[2].mib_of_bigint`）、n=10⁹ 為 `measure/routes017.json#memory_pillar.rows[3].bytes_of_bigint`，兩列的 n 值分別為 `measure/routes017.json#memory_pillar.rows[2].n` 與 `measure/routes017.json#memory_pillar.rows[3].n`，由 measure/routes017_measure.py 的 `bigint_footprint()` 產生、`measure/routes017.json#memory_pillar.deterministic` 為 true。時間外推**不作結論支柱**：時間外推的對數斜率與其區間端點**每次重跑都會改變**（本 change 內先後量到三組不同的值），因此本欄**只指鍵、不記值**：`measure/routes017.json#unclean_probe.loglog_slope_central_from_minima` 與 `measure/routes017.json#unclean_probe.loglog_slope_envelope`。任何文件都不得複製這兩個鍵的字面值——本列先前正是一邊寫著這句警語、一邊把端點釘了進去，且釘的還是已過期的快照。| — | ✓ | — |
