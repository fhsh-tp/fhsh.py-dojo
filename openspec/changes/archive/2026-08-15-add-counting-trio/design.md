...
|---|---|---|
| Q1 (fact D1) | All three go to the `apcs` series; ids assigned by the scaffold | "All to the `py` basics series" conflicts with the literacy framing; "two `py` plus one `apcs`" would need two authoring templates for one batch. Side effect: fills the series' missing `easy` rung |
| Q2 (fact D2) | `marquee-display-count` discriminates by context translation only; **no cost cliff is built** | "Rewrite as an adjacent-not-equal recurrence" is no longer the source problem and exceeds `easy`; "push n to 10^18 to force modular exponentiation" makes the wrong route die by `MemoryError` rather than by timeout, which is not a clean death |
| Q3 (fact D3) | `ap-layout-plan` keeps per-side-length output for 1 through n | "Ask about a single size" loses the row-of-numbers pattern that carries the teaching value |
| Q4 (fact D4) | `ap-layout-plan` is `easy`, its page carries the k=1..5 answer table, and it does **not** reveal the 2x3 block insight | "medium plus reveal the block" and "hard with no hint at all" |
| Q5 (fact D5) | The exchange rate is fixed at twelve | "Keep base ten" — the rule is published everywhere and a student can copy one line; "read the base from input" needs prime factorisation and is `hard` for this audience |
| Q6 (fact D6) | Access point layout / marquee display / fairground token exchange | Three alternates unused. The token challenge deliberately avoids packing vocabulary because `apcs009` is already a packing challenge |
| Q7 (fact D7) | See the parameter table below; three 20-entry all-literal plans, first entry equal to the page's worked example, independent `reference_solution` each | "Push 016 to 10^18" and "drop 017 to n <= 10^6" — the latter would make more students believe the factorial is computable |
...
At n=1000 all three spellings survive with the most expensive one holding a 3.98x margin (fact E6), and the browser confirms it: all three score 20/20 there, the slowest single entry taking 706 ms (fact W2). The discrimination is unaffected: the O(n^3) route already dies from n=249 upward in the browser, so any bound above roughly 250 discriminates identically. The bound of 3000 bought no difficulty, only the knife-edge.
...
3. **The `math.factorial` route on two of the challenges does not die cleanly.** Browser measurement made this precise, and narrower than the original statement. A `math.factorial` call with a moderate argument returns quickly, and expensive work following it runs at bytecode level and *is* interrupted cleanly — a bounded variant receives a clean timeout at 5,041 ms and scores 19/20 (fact W3). What defeats the interrupt is a single C call with an argument so large that it never returns: the flag is never examined, the worker dies, and **every** result is discarded, so the student sees 0/20 with all entries marked not executed rather than the score the surviving entries earned (facts E10, G7, W4). The page recovers on the next submission (fact W5). A control submission establishes that `import math` itself is not what the sandbox guard stops — importing it and doing nothing expensive scores 20/20 (fact W6). The counter cannot see any of this: the two routes consume 577 and 98,699 operations. This is an engine-level residue of the same family as `sys.settrace(None)` and is out of scope here.

4. **A student can pass `ap-layout-plan` by interpolation, without ever reasoning about the interference rule.** The count is a quartic in the side length, so five points pin it down, and brute-forcing the five smallest boards to obtain those points costs almost nothing. Measurement confirms both the read-them-off-the-page variant and the generate-them-yourself variant score full marks (fact E12). The second variant is the important one: it means no edit to the page closes the route, because the page is not the source of the data. Recognising that the count is a low-degree polynomial and fitting it is itself mathematical reasoning of exactly the kind this challenge's difficulty label was justified on, so the route is accepted rather than fought. The small-case table was nevertheless trimmed on the maintainer's instruction, to hand over less and push students to derive more — a teaching decision, not a defence, and the specification says so explicitly.
...
**Data shape.** Each page's frontmatter carries the same fifteen keys — `layout`, `id`, `title`, `difficulty`, `category: apcs`, `type: competition`, `tags`, `description`, `algorithm`, `params`, `input_budget`, a 20-entry all-literal `testcase_plan`, `generator`, `reference_solution`, and `starter_code` set to the empty string — with no challenge carrying an extra key, missing one, or leaving `tags` empty (fact S9). The `algorithm`, `params`, `input_budget` and `testcase_plan` values are produced by `curation/assemble.py` and copied byte-for-byte; they are never hand-edited (fact C8), and the shipped slice is byte-identical to the curation fragment at 599, 791 and 651 bytes respectively (fact S4). `generator` and `reference_solution` are implemented independently of each other, along different derivation paths rather than in two spellings of one derivation (fact S3).
...
- `python3 verify/check_frontmatter_pair.py --json-out` exits zero: the shipped `generator`, the shipped `reference_solution` and the curation semantics reference agree on all twenty literals of each challenge (fact S1), and every number in each page's prose maps to a fact id (fact S5).
- `pnpm typecheck` and `pnpm lint` stay clean and the full `vitest` run shows no regression against the pre-change baseline (fact B6).
- All three `reference_solution` implementations score 20/20 in a cross-origin-isolated browser, the tightest of them finishing in 441 ms against a per-testcase deadline of 5000 ms (fact W1).
...
**An end-to-end smoke test through the real user journey passed on all three challenges (fact W7).** The catalogue page lists every challenge id and all three titles, each short-URL alias redirects to the right slug and resolves, each challenge page returns successfully with no leaked terminology, and all three reference solutions score full marks when submitted through the actual editor rather than through a harness. One challenge was additionally driven interactively — code pasted into the editor, submit button clicked, result panel read — to confirm that the affordances a student actually touches work, not only the judging path a script can reach.

**The local preview script cannot exercise the short-URL aliases (fact W8).** It runs only the site build, while the redirect table is produced by a separate step that the full production build includes. Locally every alias therefore returns not-found until that step is run by hand. This is a fidelity gap in the preview script, not a defect in the shipped site; it is recorded because the failure looks exactly like a broken alias and cost time to distinguish from one during this change's smoke test.
...
**`marquee-display-count` declares `params.nk.min: 0` while its page states 1 <= n.** The two integers n and k are declared as one parameter with `count: 2`, so they share one range and the lower bound is the union of theirs. All three `testcase_plan` values are all-literal, so the band is never used and judging is unaffected; the inconsistency would surface only if a later change switched the challenge to band-generated inputs. The four assembled keys are frozen, so this is recorded rather than fixed (fact S8).
