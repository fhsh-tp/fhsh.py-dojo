...
- [x] 4.4 量測**所有**已上線題目 spec 中記載為收編的路線在現行 20 筆計畫下的逐筆牆鐘。每條路線一律取**最便宜的合理寫法**（RCA I-16）。驗收目標：逐筆數字寫入 design，且不修改任何題目的測資。對應 spec 需求：Bypass acceptance after hunt downgrade。
  - [x] gem-blast 的 `str.replace`（只掃 `set(s)`）→ 20/20、3,115 ms
  - [x] prize-order-code 的 `math.perm` ＋ Legendre → 20/20、2,234 ms
  - [x] rank-code-backfill 的 `math.factorial` 對照組（spec 要求其失敗）→ 0/20
  - [x] `expression-eval-challenges` 的 4 條：E1 對調 eval 20/20、52 ms；R2 regex 括弧化 20/20、12 ms；N1 C 層重寫 20/20、57 ms；E3′ 冪次編碼 20/20、65 ms。此項一度被連同整條 4.4 誤標為完成而實際零量測；四條的路線檔已佚失、由 spec 名稱重新實作，故先以 `measure/verify_routes.py` 對該題自己的 20 筆 literal 驗證輸出等同 `reference_solution`，四條全部 20/20 後才量時間。
