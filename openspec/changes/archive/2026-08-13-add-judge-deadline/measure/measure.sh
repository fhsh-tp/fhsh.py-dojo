...
TB=$(agent-browser snapshot 2>&1 | grep -i "textbox" | head -1 | grep -oE "e[0-9]+" | head -1)
BTN=$(agent-browser snapshot 2>&1 | grep "提交" | head -1 | grep -oE "e[0-9]+" | head -1)
[[ -n ${TB:-} ]] || fail "editor ref not found"
[[ -n ${BTN:-} ]] || fail "submit button ref not found"
