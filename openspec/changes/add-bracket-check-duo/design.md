...
testcase_plan:
  ...
  #   band(16)；L 長陷阱 28K(17)；L 陷阱(18)；L 長陷阱 35K(19)；L stress 62KB 深巢(20)
  #   ——交錯陷阱共 7 筆（短 5＋長 2；含相鄰型與非相鄰型兩家族，A3）
...
replace 迴圈（對出貨第 20 筆混種深巢實測 62,018 ops／2.1s native、Pyodide 3.5s）與 find+切片（310,030 ops／1.4s native）皆低於 op 上限兩個數量級；牆鐘獵殺不可行（即使 20 筆全 stress ≈ 70s Pyodide < 120s 總預算；單種 `()` 形為此路線較劣上界 93,017／5.5s）→ 量化證明不可獵殺 → 收編（R2 修訂：量測基準改為出貨 literal，原數字量自單種探針形）。題面沉默（不寫成本警語也不寫不可能性承諾——1a 主考點是 A3 正確性陷阱，繞道路線得分不影響教學目標的計數器獵殺）。第 20 筆 stress literal 讓此路線成本可見（單筆 ~2.1s native／~3.5s Pyodide，A4）但不致死（120s 總預算餘裕充足）。

### D4. 1b 斷崖：op counter 逐筆爆殺（B4，R1 重設計）

R1 稽核發現原 6 筆獵殺有兩缺陷：18/19/20 與 14/15/16 逐位元組重複；且最精簡回頭掃描（單行 while，1 op/iter）下餘裕僅 1.11×。R2 再修訂：R1 版獵殺為單種零雜訊規則形，bounty 以「形狀偵測跳過掃描」投機解拿 20/20——重構為不規則混種帶雜訊家族（參數與答案見矩陣 B4）：seeded warmup＋殘留筆 U 埋 warmup 後（答案 29/27/18/23 非公式可猜）＋混種外層/內層＋雜訊撒點；每筆 lean 下限 m×2k ≥ 20M（1 op/iter 精簡變體實測最小 K19=23,190,670 仍爆殺）、四筆答案非 0 壓制常數 0 與長行跳過投機（V8 ≤16/20）。正解（reference）最大筆 K15=100,789 ops。op 爆殺為逐筆（每筆判題獨立 10M 額度），非累積總預算殺——與 1a 的 C 繞道（op 稀疏、只能靠牆鐘）機制不同，這是 1b 能建真斷崖的根本原因。C 層回頭掃描（str.rfind 系）與等長塗白 replace 迴圈（R3 bounty 實測 20/20、~51K ops）皆 op 稀疏、不可獵殺，收編為 accepted alternative（B6/B9），與 1a replace 同判例——「結構性不可用」類全稱句已按 R3 教訓逐變體檢驗後改寫。
...
- 1b 獵殺筆餘裕：按 1 op/iter 下限設計（m×2k ≥ 20M，R2 重構後實測最小 K19=23,190,670）——單行 while 已是理論下限，無更省變體空間。
- 1a 錯誤路線得分階梯（A9）：計數器 13/20、混合投機 18/20 封頂（長陷阱 17/19 攔截）；無假解可達 20/20。
- 1a soup band 的判定分布隨 block 隨機——OK literal 保底（A8），但各 block 的 NG 比例會浮動（可接受，判題抽整 block）。
- 計數器假解在隨機 NG soup 上巧合正確（A3 修訂）——其得分下壓依賴 7 筆交錯陷阱 literal（短 5＋長 2、相鄰＋非相鄰兩型）；R2/R3 後實測計數器 13/20、混合投機 18/20、壞二元組投機 16/20，若仍認過寬，追加陷阱 literal 可線性下壓。
