...
The `reference_solution` SHALL stay within 100,000 traced ops on the worst shipped entry (measured 58,276) and the `generator` within 150,000 (measured 107,501), both far below the 10,000,000 per-testcase limit (trace C1/A0). Two routes are ACCEPTED ALTERNATIVES and SHALL keep scoring 20/20: the C-level route that locates the entrance with `list.index()` and recurses over slices (measured 38,872 ops on the worst shipped entry — cheaper than the reference, because builtin work is not traced, trace C2/A3), and the mirror route that solves mode 2 by reversing both sequences, running the mode 1 rebuild and reversing the result (measured 120,475 ops, trace A12). No op cliff SHALL be built for this challenge: the pure-Python scanning variant would need an n ~ 6000 chain to exceed the limit, which violates the depth bound (trace A4). The challenge page SHALL NOT claim any route impossible, SHALL make no performance promise beyond the stated depth bound, and SHALL NOT imply that the two modes must each be implemented separately (trace A12/D6).
...
- **THEN** it completes without RecursionError (measured at most 38,871 ops across all shipped entries)
