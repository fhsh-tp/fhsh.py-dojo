...
The binding cost gate for `ap-layout-plan` SHALL be the operation counter, not the wall clock (trace E11). Two measured properties establish this and are the form in which it SHALL be stated: locally the algorithmic increment of every accepted route stays below the process start floor or within a small multiple of it, so the timing is dominated by interpreter startup rather than by the algorithm; and in the browser the slowest accepted spelling finishes in 706 ms against a per-testcase deadline of 5000 ms (trace W2). This specification SHALL NOT quote wall-clock figures to two decimal places. Those values are re-measured on every harness run and drift between runs and between interpreters — the local and browser orderings of the three accepted spellings disagree with each other — so a fixed decimal written here becomes false without anything changing. The authoritative values live in the measurement records named by the trace matrix.

The closed-form route is the REFERENCE and SHALL score 20/20 at 3,010 operations, a margin of 3,322.26 against the limit (trace E4). Row-offset counting is an ACCEPTED ALTERNATIVE and SHALL keep scoring 20/20 **in every reasonable spelling**, verified in three: an explicit inner loop at 1,008,010 operations, a generator-expression sum at 1,510,510, and an extracted helper function at 2,509,511 (trace E5). Budgeting this route from its cheapest spelling would be an error, because the route must survive rather than die; the most expensive checked spelling therefore sets the margin, which is 3.98 (trace E6).

Per-cell scanning over all eight offsets is KILLED and SHALL score 8/20, measured in a cross-origin-isolated browser, dying from the ninth entry onward. It dies against the **operation limit**, not against the wall-clock deadline: the ninth entry consumes 216,759,740 operations against the judge's per-testcase limit of 10,000,000 (trace C1), and every dying entry takes between 1,921 and 2,139 ms regardless of its input size, which is the fixed cost of exhausting the operation budget rather than the 5,000 ms deadline (trace E8). Four wrong routes SHALL score exactly 1/20, 2/20, 2/20 and 0/20: treating placements as ordered, omitting the interference subtraction, inverting the k < 3 guard, and iterating from zero (trace E9).

Polynomial interpolation is a further ACCEPTED ALTERNATIVE and SHALL keep scoring full marks (trace E12). The k-th answer is a quartic in k across the whole declared domain, so five data points determine it exactly. Two ways of obtaining those points were measured and both pass: reading values the page already publishes, and generating them from scratch by brute-forcing the smallest few side lengths, which costs only a few hundred pair comparisons. The second variant never reads anything the page prints, and it establishes that **no change to the page can close this route**. The page's worked example alone prints more values than the five required, and that example is contract-bound to match the first testcase entry byte for byte, so it cannot be removed either. Shrinking the small-case answer table is therefore a pedagogical choice about how much help to hand over, not a defence; it SHALL NOT be described as one.
...
The challenge page SHALL NOT claim any route impossible, and any performance remark it makes SHALL be limited to the measured statement about scanning every pair of cells. That remark SHALL name the ceiling it refers to as a limit on how much execution a single testcase may consume, never as a time limit, because the route dies against the operation counter (trace S7); it SHALL NOT reveal which kind of limit the system meters.
...
The plan SHALL hold twenty pairwise distinct literals, the first being `5 2`, and SHALL cover k = 0, k = n, n = 1 and n = 1000000 (trace F2). Four wrong routes SHALL score exactly 2/20, 0/20, 0/20 and 6/20: ignoring k, mistaking the free count for k, printing n - k itself, and omitting the reduction (trace F4). The route that ignores k SHALL NOT be required to reach 0/20: the plan must cover k = 0, and that route is correct precisely there, so the two requirements are mutually exclusive and the k = 0 entries are held to the minimum of two (trace F5). The route that omits the reduction scores 6/20, being correct only on the 6 entries where n - k <= 29. Its remaining entries SHALL fail in two distinct ways, and the plan SHALL keep both ways populated: on 9 entries the unreduced power exceeds the interpreter's integer-to-string digit ceiling and the route raises a conversion error, while on 5 entries it prints a shorter integer that converts cleanly and is simply the wrong answer. Treating every failing entry as a raised error is wrong arithmetic as well as wrong mechanism, since only 6 plus 9 plus 5 accounts for all 20 entries (trace F6).
...
All three pages, slugs and `algorithm` values SHALL be free of data-structure and algorithm terminology in both English and Chinese, verified over the whole page including frontmatter (trace B5). The only soft-scanner hit is the character 「馬」 twice inside the scenario noun 「跑馬燈」 on `marquee-display-count`; it is a false positive and SHALL NOT be treated as a violation (trace S6). The assembler SHALL enforce this over every fragment it emits and SHALL refuse to write any file when a term is hit, leaving all previously emitted fragments untouched (trace C7). It SHALL likewise refuse when its input contains an expected-answer key or a comment inside a testcase plan, so that neither answers nor the identity of the discriminating routes can leak into shipped frontmatter.

All three SHALL declare a `reference_solution` implemented independently of the `generator` — a different derivation path, not a second spelling of the same one (trace S3) — and the two SHALL agree with the curation semantics reference on all twenty literals of each challenge (trace S1), verified against the production pools by `scripts/content-regression.test.ts` (trace B1). Both shipped programs SHALL stay inside the operation limit; the tightest are the two `reference_solution` implementations at 2,005,018 and 2,000,012 operations, 20.05 and 20.00 percent of the limit (trace S2). All three `starter_code` fields SHALL be the empty string so the editor loads blank. No page SHALL contain a number that is not traceable to a fact id in `trace-matrix.md` (trace S5).
